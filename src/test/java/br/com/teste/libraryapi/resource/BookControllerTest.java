...
public class BookControllerTest {
	...
	public void updateBookTest() throws Exception {
		...
		Book updatingBook = Book.builder().id(1L).title("some title").author("some author").isbn(isbn).build();
		Book updatedBook = Book.builder().id(1L).title("Artur").author("As Aventuras").isbn(isbn).build();
	...
	private Book createNewBook(Long id) {
		BookDTO dto = createNewBook();
		return Book.builder().id(id).title(dto.getTitle()).author(dto.getAuthor()).isbn(dto.getIsbn())
				.build();
	}
}
